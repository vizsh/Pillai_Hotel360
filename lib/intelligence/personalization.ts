...
export type SpendCategory = "fnb" | "spa" | "other" | "none";
...
export interface NextBestAction {
  ...
  reason: string;
  cost: number;
  /** Guest-sentiment uplift, 0–1 scale — the only effect this module tracked until now. */
  uplift: number;
  /** Incremental ₹ spend if the guest takes this up — 0 for actions that are pure service
   * recovery/relationship investment rather than an upsell (VIP welcome, late checkout). */
...
export function personalizationRecommendations(state: SimState, model: ResortModel): Recommendation[] {
  ...
  for (const { g, nba } of scored) {
    out.push({
      ...
      impact: `Est. sentiment uplift +${nba.uplift.toFixed(2)}${nba.revenueUplift > 0 ? `, +₹${nba.revenueUplift.toLocaleString("en-IN")} ancillary spend` : ""} at ₹${nba.cost.toLocaleString("en-IN")} cost.`,
      action: nba.label,
      targetKind: "guest",
      targetId: g.id,
      createdAt: state.t,
      status: "pending",
      payload: { guestId: g.id, actionId: nba.id, uplift: nba.uplift, revenueUplift: nba.revenueUplift, spendCategory: nba.spendCategory },
    });
  }
  return out;
}
